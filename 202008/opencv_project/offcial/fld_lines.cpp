...
int main(int argc ,char **argv) {
	...
	/**
	 * Because of some CPU's power stratey ,it seams that the first running of an algorithm takes much longer.
	 * So here we run the algorithm 10 times to see the algorithm's processing time with sufficiently warmed-up CPU performance.
	 **/
	for (int run_count = 0 ; run_count < 10 ; run_count++ ) {
		...
		std::out << "Elapsed time for FLD" << duration_ms << " ms." << std::endl;
	}
