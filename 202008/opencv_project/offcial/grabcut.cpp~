...
private:
	...
	Rect rect;
	vector<Point> fgbPxls, bgdPxls, prFgdPxls, prBgdPxls;
	int iterCount;
};
...
void GCApplication::mouseClick(int event ,int x ,int y ,int flags ,void*) {
	...
	switch (event) {
		...
		case EVENT_LBUTTONUP:
			{
				if (rectState == IN_PROCESS) {
					...
					rectState = SET;
					setRectInMask();
					CV_Assert(bgdPxls.empty() && fgbPxls.empty() && prBgdPxls.empty() && prBgdPxls.empty());
					showImage();
				}
		...
		case EVENT_MOUSEMOVE:
			if (rectState == IN_PROCESS) {
				rect = Rect(Point(rect.x ,rect.y) ,Point(x ,y));
				CV_Assert(bgdPxls.empty() && fgbPxls.empty() && prBgdPxls.empty() && prFgdPxls.empty());
				showImage();
