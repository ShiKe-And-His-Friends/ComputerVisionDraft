...
void decomposeHomography (const string& img1Path ,const string& img2Path ,const Size& patternSize 
	...
	for (int i = 0 ; i < solutions ; i++) {
		...
		cout << "plane normal from homography decomposition:" << normals_decomp[i].t() << endl;
		cout << "plane normal at camera 1 pose:" << normal.t() << endl << endl;
	}
}
...
const char* params = 
		...
		"{ image2		| left01.jpg			| path to the desirced chessboard iamge}"
		"{ intrinsice	| left_intrinsics.yml	| path to camera intrinsices}"
