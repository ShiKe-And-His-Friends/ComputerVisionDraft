...
void MaxAndMin () {
	...
	Mat myMax ,myMin;
	max(imga ,imgb ,myMax);  // single channel
	min(imga ,imgb ,myMin);  // mulit channel
	Mat myMaxs ,myMins;
	max(imgas ,imgbs ,myMaxs);
	min(imgas ,imgbs ,myMins);
	Mat img0 = imread("/home/shike/Pictures/lenna_head_jpg_type.jpg");
	Mat img1 = imread("/home/shike/Pictures/me.png");
