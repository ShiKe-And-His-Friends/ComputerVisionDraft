...
int main(int argc ,char **argv) {
	...
	avformat_alloc_output_context2(&ofmt_ctx ,NULL ,"flv" ,"rtmp://192.168.1.152/live2/night");
	if (!ofmt_ctx) {
	...
	for (i = 0 ; i < ifmt_ctx->nb_streams ; i++) {
		...
		out_streams = avformat_new_stream(ofmt_ctx ,NULL);
		if (!out_streams) {
			fprintf(stderr ,"Failed allocating output stream.\n");
			ret = AVERROR_UNKNOWN;
			goto end;
		}
		ret = avcodec_parameters_copy(out_streams->codecpar ,in_codecpar);
		if (ret < 0) {
			fprintf(stderr ,"Failed to copy codec parameters.\n");
			goto end;
		}
		out_streams->codecpar->codec_tag = 0;
	}
