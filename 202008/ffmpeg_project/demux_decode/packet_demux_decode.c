...
int main(int argc ,char **argv) {
	...
	int ret = 0 ,got_frame;
	if (argc != 4 && argc != 5) {
		fprintf(stderr ,"Usage : %s [-refcount] input_file video_output_file audio_output_file\n API example program to show how to read frames from an input file.\n This program reads frames from a file, decode them, and writes decoded video frame to a rawvideo file named video_output_file ,and decoded audio frames to a rawaudio file named audio_output_file.\n\n If the -refcount option is specified, the program use the refrence counting frames system which allows keeping a copy of the data for longer than one decode call.\n\n" ,argv[0]);
		exit(1);
	}
	...
	if (avformat_find_stream_info(fmt_ctx ,NULL) < 0) {
		fprintf(stderr ,"Could not find stream informatin.\n");
		exit(1);
	}
	if (open_codec_context(&video_stream_idx ,&video_dec_ctx ,fmt_ctx ,AVMEDIA_TYPE_VIDEO) > 0) {
		...
		if (ret < 0) {
			fpritnf(stderr ,"Could not allocate raw video buffer\n");
			goto end;
		}
	...
	if (video_stream) {
		printf("Paly the output file with the command:\n ffplay -f rawvideo -pix_fmt %s -video_size %dx%d %s \n"
	...
	if (audio_stream) {
		...
		if (av_sample_fmt_is_planar(sfmt)) {
			...
			printf("Warning: the sample format the decoder produced is planner (%s). This sample will output the first channel only.\n"
					,packed ? packed : "?");
			sfmt = av_get_packed_smaple_fmt(sfmt);
			n_channels = 1;
		}
	}
