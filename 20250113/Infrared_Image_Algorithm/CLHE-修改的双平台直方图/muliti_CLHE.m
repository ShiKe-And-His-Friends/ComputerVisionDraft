...
function muliti_CLHE(GrayImage ,cols ,rows ,save_dir ,name)
    ...
    %计算自适应双平台
    [pixel_mapping ,valid_num] = muliti_CLHE_PixelMapping(frequencyHist ,size(base_image));
    ...
    for i=1:cols
         for j=1:rows
            idx = base_image(i,j);
            base_adpt_image(j,i) = pixel_mapping(idx+1)-1;
         end
    end
