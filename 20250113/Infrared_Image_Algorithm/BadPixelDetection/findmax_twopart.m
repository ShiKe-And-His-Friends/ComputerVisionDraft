...
mean_index = mean(sorted_data(:));

% 提取前80%和后20%的数据
first_part = sorted_data(sorted_data <= mean_index);
second_part = sorted_data(sorted_data >= mean_index);

weight1 = 0.5;

% 计算第一段线性拉伸的参数
a1 = 255 * weight1 / (max(first_part) - min(first_part));
b1 = -a1 * min(first_part);

% 计算第二段线性拉伸的参数
a2 = (255 - 255 * weight1) / (max(second_part) - min(second_part));
