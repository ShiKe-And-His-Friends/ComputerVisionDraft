...
for i = 3:rows-2
    for j = 3:cols-2
        ...
        p1 = abs(neighborhood(1,1));
        p2 = abs(neighborhood(1,2));
        p3 = abs(neighborhood(1,3));
        p4 = abs(neighborhood(2,1));
        p5 = abs(neighborhood(2,3));
        p6 = abs(neighborhood(3,1));
        p7 = abs(neighborhood(3,2));
        p8 = abs(neighborhood(3,3));
        ...
        for index = 1:length(p_number)
         ...
         avg = 1/6 * sum(remaining_p_numbers(:));
         C1 = avg + max_value - min_value + 65*64;
         C2 = avg - max_value + min_value - 65*64;
...
%保存图片
save_raw_image = uint8(pixel_weight_sheet);
