...
function AGF_CLAHE(GrayImage ,cols ,rows ,save_dir ,name)
    ...
    limit = std(double(base_image(:))) / mean(double(base_image(:)));
    
    %计算自适应双平台
    [output_clahe_image] =  CLAHE_PixelMapping([20 16],0.005 ,base_image ,save_dir + name);
...
%     filted_detail_image = detail_image.* coff;
    filted_detail_image = detail_image.* coff_Ak_image * 12;
