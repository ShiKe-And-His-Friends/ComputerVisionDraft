...
void GCoptimization::setupSmoothCostsExpansion(SiteID size,LabelID alpha_label,EnergyT *e,SiteID *activeSites)
{
	...
	for ( i = size - 1; i >= 0; i-- )
	{
		...
		/*std::vector<std::vector<double>> smoothCosts(nNum);
		concurrency::parallel_for(0, nNum, [&](int j)
		{
			int site2 = nPointer[j];
			smoothCosts[j].reserve(4);

			if (m_lookupSiteVar[site2] == -1)
			{
				smoothCosts[j].push_back(sc->compute(site, site2, alpha_label, m_labeling[site2]));
				smoothCosts[j].push_back(sc->compute(site, site2, m_labeling[site], m_labeling[site2]));
			} 
			else if (site2 < site)
			{
				smoothCosts[j].push_back(sc->compute(site, site2, alpha_label, alpha_label));
				smoothCosts[j].push_back(sc->compute(site, site2, alpha_label, m_labeling[site2]));
				smoothCosts[j].push_back(sc->compute(site, site2, m_labeling[site], alpha_label));
				smoothCosts[j].push_back(sc->compute(site, site2, m_labeling[site], m_labeling[site2]));
			}
		});
		...
		for (n = 0; n < nNum; n++)
		{
			...
			if (m_lookupSiteVar[nSite] == -1)
				addterm1_checked(e, i, smoothCosts[n][0],
					smoothCosts[n][1], weights[n]);
			else if (nSite < site)
			{
...
GCoptimization::EnergyType GCoptimization::expansion(int &iteration_number, int max_num_iterations)
{
	...
	try 
	{
		...
		else
		{
			...
			iteration_number = max_num_iterations;
