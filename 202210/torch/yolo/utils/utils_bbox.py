...
class DecodeBox():
    def __init__(self ,anchors ,num_classes ,input_shape ,anchors_mask = [ [6,7,8] ,[3,4,5] ,[0,1,2] ]):
        ...
        #----------------------------------------------------------#
        #  13x13 特征层对应anchor是 [142 ,110] ,[192,243] ,[459 ,401]
        #  26X26 特征层对应anchor是 [36,75] ,[76,55] ,[72,146]
        #  52X52 特征层对应anchor是 [12 ,16],[19,36] ,[40,28]
        # ----------------------------------------------------------#
    ...
    def decode_box(self ,inputs):
        ...
        for i,input in enumerate(inputs):
            ...
            # --------------------------------------#
            #  此时获得scaled_anchors 大小是相对特征层的
            # --------------------------------------#
            scaled_anchors = [(anchor_width / stride_w ,anchor_height /stride_h) for anchor_width ,anchor_height in self.anchors[self.anchors_mask[i]]]

            # --------------------------------------#
            #  输入的input一共有三个，他们的shape是
    ...
    def yolo_correct_boxes(self ,box_xy ,box_wh ,input_shape ,image_shape ,letterbox_image):
        ...
        if letterbox_image:
            # --------------------------------------------------------#
            #  这里求出来offset是图像有效区域相对于图像左上角的偏移情况
            #  new_shape 指的是宽高缩放情况
            # --------------------------------------------------------#
    ...
    def non_max_suppression(self,prediction ,num_classes ,input_shape ,image_shape ,letterbox_image ,conf_thres=0.5 ,nms_thres=0.4):
        ...
        for i , image_pred in enumerate(prediction):
            ...
            #  7 的内容是 x1 ,y1 ,x2 ,y2 ,obj_conf ,class_conf ,class_pred
            # --------------------------------------#
            ...
            for c in unique_labels:
                ...
                #     # 取出这一类置信度最高的，一步一步向下判断，判断重合程度是否大于nms_thres ，如果是则去掉
