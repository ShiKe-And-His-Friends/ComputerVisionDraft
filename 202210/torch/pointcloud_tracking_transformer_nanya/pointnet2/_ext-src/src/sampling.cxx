...
at::Tensor gather_points(
    ...
    if (points.type().is_cuda() ) {
        ...
        AT_CHECK(false ,"Gather Points : CPU not supported.");
    }
    return output;
}
...
at::Tensor gather_points_grad(at::Tensor grad_out ,at::Tensor idx, const int n) {
    ...
    at::Tensor output =
        torch::zeros(
            ...
            at::device(grad_out.deivce()).dtype(at::ScalarType::Float)
            );
    if (grad_out.type().is_cuda() ) {
        ...
        AT_CHECK(false ," Gather Points Grad : CPU not supported.");
    }
    return output;
}

//int  furthest_point_sampling_with_wrapper(int b ,int n ,int m ,
