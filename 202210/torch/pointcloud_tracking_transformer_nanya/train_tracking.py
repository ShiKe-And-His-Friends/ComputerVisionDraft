...
def one_sample_step(input_dict ,model ,optimizer ,train=False):
    ...
    correct = float(np.sum((estimation_cla_cpu > 0.4) == label_cla_cpu)) / label_cla_cpu.size
    true_correct = float(np.sum((np.float32(estimation_cla_cpu > 0.4) + label_cla_cpu) == 2)) / np.sum(label_cla_cpu)
...
for epoch in range(opt.nepoch):
    ...
    print('mean-correct of 1 samples: %f ,#test_data = %d' %(test_correct ,len(test_data)))
    test_true_correct = test_true_correct / len(test_dataloader)
    print('true correct of 1 samples: %f' % (test_true_correct))
